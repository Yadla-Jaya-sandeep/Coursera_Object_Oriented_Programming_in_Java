...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		// add this to track the sentence that does not have the punctuation in the end
		boolean isInSentence = false;
		// for all tokens in tokens
		for (String token : tokens) {
			// if the token is a word, count the syllables and count as numWords
			if (isWord(token))  {
		...
		if (isInSentence == true) numSentences++;
	}
